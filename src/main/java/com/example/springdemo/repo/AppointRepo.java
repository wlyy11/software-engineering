...
public interface AppointRepo extends CrudRepository<Appointment, Integer> {
    ...
    @Query("SELECT d FROM Appointment d WHERE d.status = 'WAITING' and d.restaurant.id = :restaurant_id ")
    List<Appointment> findWaitingAppointmentsByRestaurant(@Param("restaurant_id") int restaurant_id);
}
